...
impl Chunk {
    fn new(capacity: usize) -> Self {
        let layout = Layout::from_size_align(capacity, MAX_ALIGN).expect("Invalid layout");
    ...
    fn alloc(&mut self, size: usize, align: usize) -> Option<NonNull<u8>> {
        ...
        unsafe { Some(NonNull::new_unchecked(self.data.as_ptr().add(aligned_pos))) }
    }
...
impl Drop for Chunk {
    fn drop(&mut self) {
        let layout = Layout::from_size_align(self.capacity, MAX_ALIGN).expect("Invalid layout");
        unsafe {
...
pub extern "C" fn rt_arena_alloc(arena: &mut Arena, size: usize, align: usize) -> *mut u8 {
    let layout = Layout::from_size_align(size, align.clamp(1, MAX_ALIGN)).expect("Invalid layout");
...
pub extern "C" fn rt_thread_alloc(size: usize) -> *mut u8 {
    THREAD_ARENA.with(|arena| arena.borrow_mut().alloc_bytes(size).as_ptr())
}
