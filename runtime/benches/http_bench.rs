...
use phprs_runtime::http::{parse_request, Response, StatusCode};
...
fn bench_parse_simple_get(c: &mut Criterion) {
    ...
    group.bench_function("phprs", |b| {
        b.iter(|| parse_request(black_box(SIMPLE_GET)))
    });
...
fn bench_response_build(c: &mut Criterion) {
    ...
    group.bench_function("simple_ok", |b| {
        b.iter(|| {
            black_box(Response::ok().text("Hello, World!").to_bytes())
        })
    });
    ...
    group.bench_function("not_found", |b| {
        b.iter(|| {
            black_box(Response::not_found().text("Not Found").to_bytes())
        })
    });
